...
package de.wangchao.musicplayer.thumbnail;
...
public class MySimpleWebImageCache<B extends AbstractBus, M> extends CacheBase<String, Drawable> {
    ...
    public MySimpleWebImageCache(File cacheRoot, AsyncCache.DiskCachePolicy policy, int maxSize,
            B bus) {
    ...
    public void notify(String key, M message) throws Exception {
        ...
        if (status == CACHE_NONE) {
            new FetchImageTask().execute(message, key, buildCachedImagePath(key));
        } else if (status == CACHE_DISK) {
            new LoadImageTask().execute(message, key, buildCachedImagePath(key));
        } else {
            bus.send(message);
        }
    }
